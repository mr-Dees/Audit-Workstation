 ...
 *
 * Эволюция violation-additional-content.js: тот же конвейер приёма картинок,
 * та же зона вставки и то же контекстное меню — но параметризованные ключом
 * поля. Все десять полей реестра (violation-fields.js) рендерятся ОДНИМ этим
 * компонентом; «Дополнительный контент» перестал быть особенным.
 *
