...
- [ ] **Воркер LLM-моста запущен** (только при `CHAT__PROFILE=redis-bridge,…`). `redis-cli -h <REDIS__HOST> -p <REDIS__PORT> GET llm:bridge:worker:alive` возвращает JSON с непустым `targets`, покрывающим цели primary- и fallback-маршрута. Ключа нет → воркер не поднят: `scripts/datalab/llm_redis_worker.ipynb` в Jupyter DataLab, Run All. Без воркера чат отвечает штатной ошибкой недоступности LLM (troubleshooting №7).
